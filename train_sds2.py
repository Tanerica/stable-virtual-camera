...
def main(
    ...
    # Dataset
    dataset = Image_Camera(root_dir=input_imgs,cached_dir='cached_features_eval1', preset_traj=preset_traj, num_frames=num_frames, zoom_factor=zoom_factor, camera_scale=camera_scale)
    dataloader = DataLoader(dataset, batch_size=1, shuffle=True)
    test_dataset = Image_Camera(root_dir=eval_imgs,cached_dir='cached_features_eval1', preset_traj=preset_traj, num_frames=num_frames, zoom_factor=zoom_factor, camera_scale=camera_scale)
    testloader = DataLoader(test_dataset, batch_size=1, shuffle=False)
    # Model, ae, condition
    teacher = SGMWrapper(load_model(device="cpu", verbose=True).eval()).to(device)
    teacher.module = torch.compile(teacher.module)
    student = SGMWrapper(load_model(device="cpu", verbose=True)).to(device)
    student.module.input_blocks.requires_grad_(False)
    student.train()
    ...
    ae.module.decoder = torch.compile(ae.module.decoder)
    ...
    for epoch in range(num_train_epochs):
        ...
        for value_dict in dataloader:
            ...
            second_step_idx = torch.randint(36, 777, (1,), device=device)
            second_step = denoiser.idx_to_sigma(second_step_idx)
            student_sigmas = torch.cat([sigmas[0:1], second_step])
            student_sigmas = torch.sort(student_sigmas, descending=True).values
            student_sigmas = denoiser.idx_to_sigma(denoiser.sigma_to_idx(student_sigmas))
            ...
            for step_idx, sigma_step in enumerate(student_sigmas):
                ...
                teacher_step = torch.randint(36,777,(1,),device=device)
                teacher_sigma_val = denoiser.idx_to_sigma(teacher_step)
                ...
                with torch.no_grad(), torch.autocast("cuda", dtype=torch.bfloat16):   
                    ...
                    x_c = noise_predict(teacher, teacher_sigma_val, noisy_samples, c_noise, c_replace, cond, origin=True)
                    x_u = noise_predict(teacher, teacher_sigma_val, noisy_samples, c_noise, uc_replace, uc, origin=True)
                ...
                score_gradient = torch.nan_to_num(teacher_noise_pred - noise)
                #score_gradient = torch.nan_to_num(teacher_noise_pred - lora_noise_pred)
                
                # SDS loss: ||student_x0 - (student_x0 - score_gradient)||²
                target = (student_original - score_gradient).detach()
                loss_sds = 0.5 * teacher_sigma_val * F.mse_loss(student_original.float(), target.float(), reduction="mean")
                ...
                if accelerator.is_main_process:   
                    if (global_step + 1) % eval_step == 0:
                        for batch_idx, value_dict in enumerate(testloader):
                            ...
                            sigmas = denoiser.idx_to_sigma(denoiser.sigma_to_idx(sigmas))
                            ...
                            with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
                                ...
                                save_output(
                                    samples,
                                    save_path=os.path.join(f'./{log_dir}/{global_step+1}', f"first-pass-{batch_idx}"),
                                    video_save_fps=5,
                                )
            ...
            logs = {
                "loss_sds": loss_sds.detach().item(),
                "t": second_step_idx.detach().item()
            }
