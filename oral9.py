    ...
    for epoch in range(epochs):
        ...
        for value_dict in dataloader:
            ...
            for step_idx, sigma_step in enumerate(student_sigmas):
                ...
                noisy_samples = noisy_samples + eps2 * append_dims(teacher_sigma_hat ** 2 - teacher_sigma ** 2, x.ndim) ** 0.5
                # Teacher (frozen) prediction
                with torch.no_grad():
                    
                    # Teacher (frozen) prediction - no gradients needed
                    with accelerator.unwrap_model(teacher).disable_adapter():
                    ...
                    teacher_noise_pred = to_d(noisy_samples, teacher_sigma_hat, teacher_denoised)
                    
                    
                    lora_denoised = denoiser(accelerator.unwrap_model(teacher), *guider.prepare_inputs(noisy_samples, teacher_sigma_hat, cond, uc), num_frames=T)
                    lora_denoised = guider(lora_denoised, teacher_sigma_hat, guidance_scale, **guider_kwargs)
                    lora_noise_pred = to_d(noisy_samples, teacher_sigma_hat, lora_denoised)
                  
                # Compute VSD gradient in score space
                # Score gradient: ∇ log p(z|y) = (teacher_score - lora_score) * σ²
                teacher_score = -(noisy_samples - teacher_noise_pred) / teacher_sigma_expanded**2
                lora_score = -(noisy_samples - lora_noise_pred) / teacher_sigma_expanded**2
                
                # VSD gradient: σ² * (teacher_score - lora_score)
                score_gradient = teacher_sigma_expanded**2 * (teacher_score - lora_score)
                ...
                # Sample teacher sigma (different from student sigma for VSD)
                teacher_sigma = (84.9163 - 0.1634) * torch.rand_like(s_in) + 0.1634
                teacher_sigma_expanded = append_dims(teacher_sigma, latent.ndim)
                teacher_sigma_hat = teacher_sigma + 1e-6
                # Add noise for teacher prediction
                
                noisy_samples = latent + noise * teacher_sigma_expanded
                
                lora_denoised = denoiser(teacher, *guider.prepare_inputs(noisy_samples, teacher_sigma_hat, cond, uc), num_frames=T)
                # breakpoint()
                lora_pred = guider(lora_denoised, teacher_sigma_hat, guidance_scale, **guider_kwargs)
                noise_predict = to_d(latent, teacher_sigma_hat, lora_pred)
                
                
                loss_lora = F.mse_loss(noise_predict, noise, reduction='mean')
                ...
                if accelerator.sync_gradients:
                    global_step += 1
                    accelerator.log({"train_loss_sds": loss_vsd, "lora_loss": loss_lora}, step=global_step
                                    )
                        ...
                        if (global_step + 1) % eval_step == 0:
                            ...
                            with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
                                for i in tqdm(range(num_sigmas - 1), total=num_sigmas - 1, desc="Sampling", leave=False):
                                    ...
                                    eps = torch.randn_like(x) 
                                    x = x + eps * append_dims(sigma_hat**2 - sigma**2, x.ndim) ** 0.5
                                    denoised = denoiser(accelerator.unwrap_model(student), x, sigma_hat, cond, num_frames=T)
                                    d = to_d(x, sigma_hat, denoised)
                                    dt = append_dims(next_sigma - sigma_hat, x.ndim)
                                    x = x + dt * d
