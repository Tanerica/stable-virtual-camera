...
def main(
    ...
    traj_prior_c2ws = all_c2ws[[round(ind) for ind in anchor_indices]]
    traj_prior_Ks = all_Ks[[round(ind) for ind in anchor_indices]]

    # Conditioning
    all_imgs_np = (
        torch.nn.functional.pad(input_imgs, (0, 0, 0, 0, 0, 0, 0, num_targets), value=0.0).numpy()
        * 255.0
    ).astype(np.uint8)
    image_cond = {
        "img": all_imgs_np,
    ...
    # NOTE: redundant traj_prior_Ks normalization removed
