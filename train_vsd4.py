        ...
        for value_dict in dataloader:
            ...
            for step_idx, sigma_step in enumerate(student_sigmas):
                ...
                if accelerator.is_main_process: 
                    ...
                    #print(f"[Before cleanup] Cached:    {torch.cuda.memory_reserved() / 1024**3:.2f} GB")
                    free, total = torch.cuda.mem_get_info()
                    print(f"Free before {device}: {free/1024**3:.2f} GB, Total: {total/1024**3:.2f} GB")
                # Backward pass
                ...
                if accelerator.is_main_process: 
                    ...
                    free, total = torch.cuda.mem_get_info()
                    print(f"Free after: {free/1024**3:.2f} GB, Total: {total/1024**3:.2f} GB")

            if accelerator.sync_gradients:
                global_step += 1
                accelerator.log({"train_loss_vsd": loss_vsd}, step=global_step
                                )
