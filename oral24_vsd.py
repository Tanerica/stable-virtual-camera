    ...
    for epoch in range(num_train_epochs):
        ...
        for value_dict in dataloader:
            ...
            for step_idx, sigma_step in enumerate(student_sigmas):
                ...
                student_original = student(input_c * c_in, c_noise, cond, num_frames=T) * -sigma2 + input_c
                ...
                teacher_sigma_expanded = append_dims(teacher_sigma, noisy_samples.ndim)
                ...
                with torch.no_grad():
                    ...
                    with accelerator.unwrap_model(teacher).disable_adapter():
                            teacher_denoised = denoiser(accelerator.unwrap_model(teacher), *guider.prepare_inputs(noisy_samples, teacher_sigma, cond, uc), num_frames=T)
                    ...
                    lora_denoised = denoiser(accelerator.unwrap_model(teacher), *guider.prepare_inputs(noisy_samples, teacher_sigma, cond, uc), num_frames=T)
                    lora_denoised = guider(lora_denoised, teacher_sigma, guidance_scale, **guider_kwargs)
                    lora_noise_pred = to_d(noisy_samples, teacher_sigma, lora_denoised)
                ...
                lora_denoised = denoiser(teacher, *guider.prepare_inputs(noisy_samples, teacher_sigma, cond, uc), num_frames=T)

                lora_pred = guider(lora_denoised, teacher_sigma, guidance_scale, **guider_kwargs)
                lora_noise_pred = to_d(noisy_samples, teacher_sigma, lora_pred)
                loss_lora = F.mse_loss(lora_noise_pred, noise, reduction="mean")
                        ...
                        for batch_idx, value_dict in enumerate(testloader):
                            ...
                            with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
                                for i in tqdm(range(num_sigmas - 1), total=num_sigmas - 1, desc="Sampling", leave=False):
                                    ...
                                    denoised = student(input_c * c_in, c_noise, cond, num_frames=T) * -sigma2 + input_c
