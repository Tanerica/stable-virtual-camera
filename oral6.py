...
def main(
    ...
    for epoch in range(epochs):
        ...
        for value_dict in dataloader:
            ...
            distill_shape = torch.tensor([0, 1, 6, 11, 16]).to(device)
            ...
            cond_x = {
                "crossattn": c_crossattn[distill_shape],
                "replace": c_replace[distill_shape],
                "concat": c_concat[distill_shape],
                "dense_vector": c_dense_vector[distill_shape],
            }
            latent_x = latent[distill_shape]
            s_in_x = s_in[distill_shape]
            next_sigma_x = next_sigma[distill_shape]
            for step_idx, sigma_step in enumerate(student_sigmas):
                ...
                sigma = s_in_x * sigma_step
                
                sigma_hat = sigma * (gamma + 1.0) + 1e-6
                x = latent_x * torch.sqrt(1.0 + sigma_step ** 2.0)
                
                denoised = denoiser(student, x, sigma_hat, cond_x, num_frames=T)
        
                # Get student's original sample prediction
                d = to_d(x, sigma_hat, denoised)
                dt = append_dims(next_sigma_x - sigma_hat, x.ndim)
                ...
                target = (student_original - score_gradient[distill_shape]).detach()
