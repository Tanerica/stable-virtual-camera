    ...
    for epoch in range(num_train_epochs):
        ...
        for batch_idx, value_dict in enumerate(dataloader):
            ...
            for step_idx, sigma_step in enumerate(student_sigmas):
                ...
                with torch.no_grad(), torch.autocast('cuda', dtype=torch.bfloat16):
                    teacher_denoised = denoiser(teacher, *guider.prepare_inputs(noisy_samples, teacher_sigma_hat, cond, uc), num_frames=T)
                    teacher_denoised = guider(teacher_denoised, teacher_sigma_hat, guidance_scale, **guider_kwargs)
                    d_teacher = to_d(noisy_samples, teacher_sigma_hat, teacher_denoised)
                    dt_teacher = append_dims(next_sigma - teacher_sigma_hat, noisy_samples.ndim)
                    teacher_original = noisy_samples + dt_teacher * d_teacher
                    ...
                    if accelerator.is_main_process:   
                        if (global_step + 1) % eval_step == 0:
                            ...
                            with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
                                ...
                                save_output(
                                    samples,
                                    save_path=os.path.join(f'./log_sds/{global_step+1}', "first-pass"),
                                    video_save_fps=5,
                                )
