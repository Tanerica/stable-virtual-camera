        ...
        for batch_idx, value_dict in enumerate(dataloader):
            ...
            for step_idx, sigma_step in enumerate(student_sigmas):
                ...
                with torch.no_grad(), torch.autocast('cuda', dtype=torch.bfloat16):
                    ...
                    d_teacher = to_d(noisy_samples, sigma_hat, denoised)
                    dt_teacher = append_dims(next_sigma - sigma_hat, noisy_samples.ndim)
                ...
                print(f"Step {sigma_step.item():.4f}, SDS Loss: {loss_sds.item():.6f}")
